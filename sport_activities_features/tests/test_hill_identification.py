...
class TestHillIdentification(TestCase):
    ...
    def test_avg_altitude_correct(self):
        self.assertAlmostEqual(self.top.avg_altitude_of_hills(self.activity['altitudes']), 145.0, places=1)        

    def test_avg_ascent_correct(self):
        self.assertAlmostEqual(self.top.avg_ascent_of_hills(self.activity['altitudes']), 91.6, places=1) 
        
    def test_distance_of_hills_correct(self):
        self.assertAlmostEqual(self.top.distance_of_hills(self.activity['positions']), 20.0, places=1) 
        
    def test_share_of_hills_correct(self):
        distance_hills = self.top.distance_of_hills(self.activity['positions'])
        self.assertAlmostEqual(self.top.share_of_hills(distance_hills, self.activity['total_distance']), 0.17, places=2)
