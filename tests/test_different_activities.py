...
class TestCrossCountryTCXFile(TestCase):
    def setUp(self):
	# test file is taken from https://github.com/firefly-cpp/tcx-test-files
        filename = os.path.join(os.path.dirname(__file__), 'data', 'cross-country-skiing_activity_1.tcx')
        self.tcx_file = TCXFile()
        self.data = self.tcx_file.extract_integral_metrics(filename)
...
class TestWalkingTCXFile(TestCase):
    def setUp(self):
	# test file is taken from https://github.com/firefly-cpp/tcx-test-files
        filename = os.path.join(os.path.dirname(__file__), 'data', 'walking_activity_1.tcx')
        self.tcx_file = TCXFile()
        self.data = self.tcx_file.extract_integral_metrics(filename)
...
class TestPoolSwimmingTCXFile(TestCase):
    def setUp(self):
	# test file is taken from https://github.com/firefly-cpp/tcx-test-files
        filename = os.path.join(os.path.dirname(__file__), 'data', 'pool_swim-activity_1.tcx')
        self.tcx_file = TCXFile()
        self.data = self.tcx_file.extract_integral_metrics(filename)
